...
function LoginForm() {
  ...
  const handleOAuth = async () => {
    if (!supabase) {
      setStatus("Configura Supabase per attivare il login.");
      return;
    }
    ...
    setStatus("Reindirizzamento a Google...");
  };
  ...
  const handleEmail = async (e: React.FormEvent) => {
    ...
    setStatus("Account creato. Controlla la tua email per confermare.");
    setLoading(false);
  };
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleEmail} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-full bg-zinc-900 py-2.5 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-50"
            >
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="flex min-h-[60vh] items-center justify-center">Caricamento...</div>}>
      <LoginForm />
    </Suspense>
  );
}
